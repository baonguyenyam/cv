...
if (process.env.NODE_ENV !== 'production') {
  ...
  request({
    ...
    fs.writeFile('./docs/index.html', htmlContent, function (error) {
      /* handle error */
    });
  });
  request({
    ...
    var htmlContent = body;
    fs.writeFile('./docs/en/index.html', htmlContent, function (error) {
      /* handle error */
    });
  });
  request({
    ...
    var htmlContent = body;
    fs.writeFile('./docs/game/index.html', htmlContent, function (error) {
      /* handle error */
    });
  });
}
...
app.use('/cv/game/', router); // handling 404 errors
...
app.use(function (err, req, res, next) {
  ...
  res.render('500.pug', {
    ...
    desc: "Application is shutting down on the web server."
  });
});
app.use(compression());
...
function PugCom(a, b) {
  ...
  parseFiles = function parseFiles(dirname) {
  ...
  writeToOutput = function writeToOutput(fn, fnName) {
    var fnString;
    var id = makeid(10);
